<x-app-dashboard 
    ...
    subtitle="Upload a new property document to your library"
    role="landlord">
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                <h2 class="text-xl font-bold text-white">Upload New Document</h2>
            </div>
            
            <form action="{{ route('landlord.documents.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                
                <!-- Document Title -->
                <div>
                    <label for="title" class="block text-sm font-semibold text-gray-700 mb-2">Document Title *</label>
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('title') border-red-500 @enderror">
                    @error('title')
                ...
                <div>
                    ...
                    <select id="document_type" name="document_type" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('document_type') border-red-500 @enderror">
                        ...
                        <option value="ownership_deed" {{ old('document_type') == 'ownership_deed' ? 'selected' : '' }}>Ownership Deed</option>
                        <option value="tax_receipt" {{ old('document_type') == 'tax_receipt' ? 'selected' : '' }}>Tax Receipt</option>
                        <option value="noc" {{ old('document_type') == 'noc' ? 'selected' : '' }}>NOC (No Objection Certificate)</option>
                        <option value="insurance" {{ old('document_type') == 'insurance' ? 'selected' : '' }}>Insurance Document</option>
                        <option value="survey_plan" {{ old('document_type') == 'survey_plan' ? 'selected' : '' }}>Survey Plan</option>
                        <option value="building_approval" {{ old('document_type') == 'building_approval' ? 'selected' : '' }}>Building Approval</option>
                        <option value="utility_bill" {{ old('document_type') == 'utility_bill' ? 'selected' : '' }}>Utility Bill</option>
                        <option value="lease_agreement" {{ old('document_type') == 'lease_agreement' ? 'selected' : '' }}>Lease Agreement</option>
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="file" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                    ...
                                    <input id="file" name="file" type="file" class="sr-only" required accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                                </label>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="expiry_date" name="expiry_date" value="{{ old('expiry_date') }}" min="{{ date('Y-m-d', strtotime('+1 day')) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('expiry_date') border-red-500 @enderror">
